...
class PyADB:
    ...
    def swipe_tap(self, start, end):
        """滑动, (x, y) 格式"""
        cmd = f"adb -s {self.device_serial} exec-out input swipe {start[0]} {start[1]} {end[0]} {end[1]}".split(
        )
        result = _sysrun(cmd)
        if result.returncode != 0:
            raise SwipeError(result.stderr.decode.strip())
